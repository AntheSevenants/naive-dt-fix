df <- read.csv("RoodGroenAnthe.csv", sep=",")

all_participles <- unique(df$participle)
...
for (participle in all_participles) {
  ...
  # If the alternative is not found in the dataset, definitely don't consider it
  ...
  if (participle_alternative_frequency > participle_frequency) {
    df$participle <- gsub(participle, alternative_form, df$participle)
